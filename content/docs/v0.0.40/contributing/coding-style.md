---
title: "Coding Style_content/Docs/V0.0.40/Contributing"
date: 2020-06-10T14:34:52+09:00
draft: true
weight: 0
menu:
  contributing:
...
Please also read the [Contribution guideline](/docs/v0.0.40/contributing/contributing-guide) before you start contributing to Vald.
...
To initialize complex structs, we can use [functional option pattern](https://dave.cheney.net/2014/10/17/functional-options-for-friendly-apis). Please read [server.go](/docs/v0.0.40/internal/servers/servers.go) and [option.go](/docs/v0.0.40/internal/servers/option.go) for the reference implementation.
...
All errors should define in [internal/errors package](/docs/v0.0.40/internal/errors). All errors should be start with `Err` prefix, and all errors should be handle if possible.

Please use [internal/errgroup](/docs/v0.0.40/internal/errgroup) for synchronized error handling on multi-goroutine processing.

### Logging

We define our own logging interface in [internal/log package](/docs/v0.0.40/internal/log). By default we use [glg](https://github.com/kpango/glg) to do the logging internally.
